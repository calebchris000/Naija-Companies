<script lang="ts">
    ...
    async function getOrganization() {
        ...
        if (response.status !== 200) {
            notification.error({
                text:
                    response.data?.message ?? "Could not get organization info",
            });
            return;
        }
    ...
    function handleSort() {
        ...
        if (filter_by.year && filter_by.month) {
            sorted_review = reviews.filter((r) => {
                ...
                return (
                    Number(filter_by.year) === r_year &&
                    filter_by.month === r_month
                );
            });
        } else {
            if (filter_by.year) {
        ...
        if (!status.includes(response.status)) {
            notification.error({
                text:
                    ...
                    "Could not react to review. Try again",
            });
            return;
        }
    ...
    >
        ...
        <div class="flex flex-col gap-4 w-full">
            ...
            <div class="flex flex-col gap-4 lg:h-[75vh] overflow-y-scroll">
                ...
                {:else}
                    <span
                        class="text-3xl text-gray-500 font-medium translate-y-10"
                        >Be the first to <button
                            class="text-yellow-500 underline"
                            on:click={handleAddReview}
                            type="button">add a review.</button
                        >
                    </span>
                {/if}
            </div>
