<script lang="ts">
    ...
    function handleCapital(d: { company: string; id: string }) {
        ...
        const selected_capital = local_storage.getItem(
            "selected_capital",
            false,
        );
    ...
    {:else}
        ...
        >
                ...
                >
                    ...
                    <span class="flex flex-col"
                        >Organizations In Selected Capital <Link
                            ...
                            to="/home/organization/add"
                            >Add an existing organization</Link
                        >
                    </span>
                </div>
            {/if}
            ...
            {#if !is_mobile}
                <div
                    style=""
                    class="bg-gray-100 p-4 h-[80vh] overflow-y-scroll"
                >
                    {#if organization_status === "not_active"}
                        <span
                            class="font-medium text-gray-400 pt-10 text-2xl mx-auto"
                    ...
                    {:else}
                        <div class="grid grid-cols-3 gap-4 px-0 w-full">
                            ...
                            {:else}
                                <div
                                    class="flex flex-col col-span-3 px-10 w-full gap-10"
                                >
                                    <span
                                        class="font-medium text-gray-400 pt-10 text-2xl"
                                        >There are no organizations listed on
                                        this capital.</span
                                    >
                                    {#if is_mobile}
                                        <button
                                            ...
                                            type="button"
                                            >Add An Existing Organization</button
                                        >
                                    {/if}
                                </div>
                            {/if}
